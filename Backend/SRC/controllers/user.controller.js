...
const updateProfile = asyncHandler(async (req, res) => {
  ...
  try {
    ...
    const updatedUser = await User.findByIdAndUpdate(userId, { $set: { fullName, username, email } }, { new: true, runValidators: true });
    if (!updatedUser) {
      res.status(500).json(new ApiResponse(500, null, "Having trouble to update profile"))
    ...
    if (error.name === "MongoServerError" && error.code === 11000) {
      ...
      res.status(500).json(new ApiResponse(400, null, `${duplicateField} is already in use`));
...
const getCounts = (req, res) => {
  return res.status(200).json(new ApiResponse(200, { addApiCount, updateApiCount }, "API counts retrieved successfully"));
};

module.exports = { userRegistration, updateProfile, getAllUsers, deleteUser, getCounts };
